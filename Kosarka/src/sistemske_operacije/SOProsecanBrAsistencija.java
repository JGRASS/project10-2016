 ...
 * 
 * */
public class SOProsecanBrAsistencija {
	/**Metoda koja vraca broj prosecan broj asistencija
	 * @param timovi lista timova
	 * @param t tim za koji zelimo da dobijemo povratnu informaciju o asistencijama 
	 * @return prosecan broj asistencija
	 */
	public static double izvrsi(List<Tim> timovi, Tim t) {
		...
		for (int i = 0; i < timovi.get(SOPozicijaTima.izvrsi(timovi,t)).getUtakmice().size(); i++) {
			ukupno += timovi.get(SOPozicijaTima.izvrsi(timovi,t)).getUtakmice().get(i).getStatistika().getAsistencije();
		}
